# Kangaroo Hopping – Direct-Collocation Predictive Simulation (MATLAB + CasADi)

This repository contains MATLAB code for a **predictive simulation of a hopping kangaroo** (template also suitable for a “hopping robot”). The motion is generated by solving a **trajectory optimization** problem with **direct collocation** (backward Euler scheme) using **CasADi**’s Opti stack and the **IPOPT** nonlinear solver.
...
- **Physics-based model**: pelvis + three-segment leg (femur, tibia, foot/toes), configurable masses, inertias, lengths, and a **toe–femur elastic element** (spring) with damping.
...
2. **Choose environment & mesh**
   - `selected_gait`: `'nominal' | 'on_the_moon' | 'on_mars'` → sets gravity `g`.
...
5. **Solve**
   - Problem solved with IPOPT via `opti.solver('ipopt', optionssol)`; reasonable tolerances and “acceptable” early-exit settings provided.
...
```
.
├─ kangaroo_hop_main.m           % the script you pasted here
├─ eulerIntegrator.m             % backward Euler residuals for collocation
├─ getModelConstraintErrors.m    % returns implicit dynamics residuals f(q,dq,ddq,T,F)=0
├─ getJointPositions.m           % forward kinematics for animation
├─ generateAnimation.m           % simple line-figure animator
└─ /casadi-.../                  % your CasADi install (external)
```
...
- **Convergence checks**: Decrease `dt = T/N` (increase `N`) and verify trajectories/torques don’t change materially → mesh convergence.
...
- CasADi: Andersson et al., *CasADi – A software framework for nonlinear optimization and optimal control*, **Math. Prog. Comp.** (2019).
- IPOPT: Wächter & Biegler, *On the Implementation of a Primal-Dual Interior Point Filter Line Search Algorithm for Large-Scale Nonlinear Programming*, **Math. Prog.** (2006).
...
**Q: MATLAB can’t find CasADi or `import casadi.*` fails.**  
A: Update the `addpath` to your CasADi folder and ensure it matches your MATLAB version.

**Q: IPOPT fails or returns “infeasible”**  
